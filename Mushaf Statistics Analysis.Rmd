...
author: "Quran Universal Database Project"
date: "`r Sys.Date()`"
output: 
  html_notebook:
    toc: true
    ...
    if (length(pages_table) > 0) {
      ...
      tryCatch({
        ...
        if ("last_word_id" %in% columns) {
          max_word_query <- paste("SELECT MAX(last_word_id) as max_word_id FROM", table_name, "WHERE last_word_id IS NOT NULL")
        ...
        # METHOD 2: Precise count by summing words per line
        if ("first_word_id" %in% columns && "last_word_id" %in% columns) {
          # Calculate words per line: (last_word_id - first_word_id + 1)
          precise_count_query <- paste("SELECT SUM(last_word_id - first_word_id + 1) as total_words FROM", table_name, 
        ...
        # Get word ID range
        if ("first_word_id" %in% columns && "last_word_id" %in% columns) {
          range_query <- paste("SELECT MIN(first_word_id) as min_id, MAX(last_word_id) as max_id FROM", table_name,
                              "WHERE first_word_id IS NOT NULL AND last_word_id IS NOT NULL")
        ...
        # Calculate words per page (using correct page column)
        if (!is.null(page_col) && "first_word_id" %in% columns && "last_word_id" %in% columns) {
          words_per_page_query <- paste("SELECT", page_col, "as page_num,", 
                                       "SUM(last_word_id - first_word_id + 1) as words",
                                       "FROM", table_name,
                                       "WHERE first_word_id IS NOT NULL AND last_word_id IS NOT NULL",
        ...
        # Surah analysis (if surah column exists)
...
if (nrow(surah_page_mapping) > 0) {
  ...
  surah_clean <- surah_page_mapping %>%
    mutate(
      # Clean surah identifiers (could be number or name)
      surah_id = case_when(
        !is.na(surah_number) ~ paste("Surah", surah_number),
        !is.na(surah_name) ~ str_trim(surah_name),
        TRUE ~ "Unknown"
      ),
      # Extract page ranges
